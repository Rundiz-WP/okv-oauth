<?php
/**
 * Edit account page, change email button.
 * 
 * This will be use in WooCommerce.
 * 
 * @package okv-oauth
 * 
...
<div class="rd-oauth-form">
    ...
    <div class="rd-oauth-links-admin">
        <?php
        if (isset($okv_oauth_options['login_method'])) {
            if ('1' === $okv_oauth_options['login_method'] || '2' === $okv_oauth_options['login_method']) {
                // oauth+wp login (1), oauth only (2)
        ?> 
        <p><?php esc_html_e('Change an email by click on the OAuth button and login to OAuth provider with the email you want.', 'okv-oauth'); ?></p>
        <?php
            }// endif;
        }// endif; isset($okv_oauth_options['login_method']);
        ?> 
        <?php
        ...
        <div class="rd-oauth-link oauth-<?= esc_attr($okv_oauth_providerKey); ?>">
            <a class="rd-oauth-button <?= esc_attr($okv_oauth_providerKey); ?>" href="<?php echo esc_url($okv_oauth_OAuthClass->getAuthUrl(get_edit_user_link() . '?rdoauth=' . $okv_oauth_providerKey)); ?>">
                <i class="<?php echo esc_attr($okv_oauth_OAuthClass->getIconClasses()); ?>"></i> <?php esc_html_e('Change email', 'okv-oauth'); ?>
            </a>
        </div>
        <?php
            }// endforeach;
